...
The parameters of `@Returns()` are almost identical to those of `@Logged()`. Therefore, you can use `@Returns()` in a similar way to `@Logged()`.

However, one difference is that unlike `@Logged()`, `@Returns()` doesn't require a `name`.

* If used without any parameters, it outputs the entire return value.
...
@LoggedInjectable()
export class AppService {
  @Returns() // We want to log the return value.
  createNewUser(
    @Logged({
...
* You can also use the name and options parameters of `@Logged()`.
...
1. You can see that the logs include an **ID**. This ID is uniquely generated for each Request. It allows you to track individual Requests when logs from multiple simultaneous Requests are mixed.
2. You can see the **function call flow** for a Request, similar to a traceback. If an error occurs in your service, you can immediately identify which function execution caused the error with just one Request ID.
